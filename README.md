...
- **Files (RAG)** — attach PDFs, Word, text/CSV, images or videos in chat (they go to your shared library, used by every agent) or add them to one agent in its settings. A video is read twice over: its sound is transcribed and four stills are described, so both what is said in it and what it shows become searchable text (needs `ffmpeg` on the machine). Text is split into chunks and indexed with local embeddings (free, runs on this machine) plus keyword search. Relevant chunks are added to every question.
...
- **Reminders** — a to-do with a reminder time, or a routine falling due, buzzes the phone at that moment for anyone who has notifications on. Tapping it opens the list. Several things due at once arrive as one notice, not a pile. A server that was off overnight does not wake anyone to yesterday: anything more than six hours late is skipped, and stays on the list as it always did. The badge and the first-screen line are unchanged and keep working on their own if the timer ever stops — the buzz is an addition, not the mechanism.
- **Notifications** — Team chat shows a **Turn on notifications** row, and the bell beside sign-out in the menu turns them off again. Each device is asked separately, so a phone and a laptop are two decisions. A notice is only sent to people who do *not* have Jarvis open — anyone watching already saw the message arrive — and tapping it opens that chat. Nothing is notified but team messages: to-dos and routines still surface as a badge and a line on the first screen, which needs no permission and cannot quietly stop working.
...
Notifications on an iPhone need iOS 16.4 or newer **and** the Home Screen icon — they do not work in Safari itself, however the page is opened. Everyone already using Jarvis keeps their icon; the app updates itself and the row appears in Team chat next time they open it. Nobody has to add it again.
